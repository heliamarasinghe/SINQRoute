...
void TrafficGenerator::generatePeriodicTraffic(int currTslot, int bkup, bool shrdAsBase){
	...
	try
	{
		...
		if(TRAF_DBG)cout<<"\tRetaining and Dropping previously embedded requests"<<endl;
		if(TRAF_DBG)cout<<"\tDropped IaasReq count:"<<nb_dropped_vnp_requests<<" \tDropped IaaReq = ";
		...
		if(TRAF_DBG)cout<<"\tCreating new IaaS requests for this time period"<<endl;
		if(TRAF_DBG)cout<<"\tNewly added IaasReq count:"<<nb_added_vnp_requests;
		...
		if(TRAF_DBG)substrate_graph_printing(Vect_Substrate_Graph, env, NB_NODE);
		...
		if(TRAF_DBG)cout<<"nb_candidate_embdedding_nodes:"<<nb_candidate_embdedding_nodes<<endl;
		...
		if(TRAF_DBG)cout<<"COUNT_PATH:"<<COUNT_PATH<<endl;
		...
		cout<<"\tperiod     : "<<current_period<<endl;
		cout<<"\ttotVlCount : "<<NB_REQUEST<<endl;
		cout<<"\trtndVlCount: "<<NB_RESERVED_REQUEST<<endl;
		cout<<"\tnewVlCount : "<<NB_ADD_REQUEST<<endl;
		cout<<"\tperiod\tvnp_id\tvlinkId\tsrc\tdest\tQoS\tbid"<<endl;
		...
		cout<<"\tDONE: Traffic Generation for TIME SLOT: "<<currTslot<<endl;
	}

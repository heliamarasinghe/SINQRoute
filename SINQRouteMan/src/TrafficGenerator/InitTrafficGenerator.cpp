...
void TrafficGenerator::generateInitTraffic(){
	...
	cout<<"\n\t*** Executing Initial Traffic Generator ***"<<endl;
	IloEnv env;
	try
	{
		// Files being read
		...
		if(TRAF_DBG1)cout<<"\tNB_REQUEST:"<<NB_REQUEST<<"\tNB_VNP_NODE:"<<NB_VNP_NODE<<endl;
		...
		for(i=0;i<NB_VNP;i++){
			...
			h=0;
			if(TRAF_DBG1)cout<<"debug"<<vect_VNP_node_location[0]<<endl;
			for(k=0; k<rand_nb_node;k++){
				...
				j=0;
				if(TRAF_DBG1)cout<<"nb: "<<nb_loc<<endl;
				while(j < nb_loc){
					...
					if(TRAF_DBG1)cout<<"embedding_node_location: "<<embedding_node_location<<endl;
		...
		for(j=0;j<NB_VNP;j++){
			...
			vnp_id = j+1;
			if(TRAF_DBG1) cout<<"\tVNP Id:"<<vnp_id<<"\tcvnp_nb_node:"<<cvnp_nb_node<<"\tcvnp_nb_link:"<<cvnp_nb_link<<endl;
			...
			while (i < upper_bound){
				...
				if ((find==0)&& (src!=dest)){
					...
					if(TRAF_DBG1) cout<<"\n\tsrc:"<<src<<"\tdest:"<<dest<<"\tvirtual link id:"<<virtual_link_id<<endl;
		...
		// =======================================================================================================================
		cout<<"\n                    Generating substrate bw unit cost and node costs and Saving in FILE 5                 "<<endl;
		// =======================================================================================================================
		//---------------------------------------------------------------------
		//       FILE 5:  Generating and Saving substrate bw unit cost and node costs       -
		//---------------------------------------------------------------------
		...
		for(j=0;j<NB_LINK;j++){
			...
			if(TRAF_DBG2) cout<<"\tBW unit cost of link:"<<j+1<<": "<<link_unit_cost<<endl;
		}
		for(j=0;j<NB_NODE;j++){
			...
			if(TRAF_DBG2) cout<<"\tCPU unit cost of node:"<<j+1<<": "<<node_cpu_unit_cost<<endl;
		}
		for(j=0;j<NB_NODE;j++){
			...
			if(TRAF_DBG2) cout<<"\tGPU unit cost of node:"<<j+1<<": "<<node_gpu_unit_cost<<endl;
		}
		for(j=0;j<NB_NODE;j++){
			...
			blade_unit_cost = 1 + (IloInt)((double)rand() / ((double)RAND_MAX + 1) * MAX_BLADE_UNIT_COST);
			if(TRAF_DBG2) cout<<"\tBlade unit cost of MSPP Blade:"<<j+1<<": "<<blade_unit_cost<<endl;
		}
		for(j=0;j<NB_NODE;j++){
			...
			if(TRAF_DBG2) cout<<"\tRAM unit cost of node:"<<j+1<<": "<<node_ram_unit_cost<<endl;
		}
		for(j=0;j<NB_NODE;j++){
			...
			if(TRAF_DBG2) cout<<"\tStorage unit cost of node:"<<j+1<<": "<<node_storage_unit_cost<<endl;
		}
		file5.close();


		// =======================================================================================================================
		cout<<"\n         Smart BID generation with the knowledge of shortest path costs between embedding locations       "<<endl;
		// =======================================================================================================================
		...
		if(TRAF_DBG3) cout<<"\n\tnb_candidate_embdedding_nodes:"<<nb_candidate_embdedding_nodes<<endl;
		...
		if(TRAF_DBG3) cout<<"COUNT_PATH:"<<COUNT_PATH<<endl;

		if(TRAF_DBG3) printing_meta_path(Path_Vect, COUNT_PATH, env);
		...
		for(k=0;k<NB_REQUEST;k++){
			...
			cout<<"\t"<<vnp_id<<"\t"<<k+1<<"\t"<<class_QoS <<"\t"<< bw<<"\t"<< selected_src_dest_cost<<" ( "<<min_src_dest_cost <<" ~ "<<max_src_dest_cost<<" )\t\t"<<selected_path_unit_cost<<" ( "<<min_path_unit_cost<<" ~ "<<max_path_unit_cost<<" )\t\t"<<selected_ppp<<"\t\t"<<bid<<endl;

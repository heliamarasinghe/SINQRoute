 ...
 try
  {
   ...
   //********************************************************************************************************
    ...
    IloInt NB_VNODE_SNODE=0, NB_TOTAL_VNP=0, NB_TOTAL_VNP_NODE=0, x_VECT_LENGTH=0, NB_PATHS=0, NB_VNP=0, NB_VNP_NODE=0, NB_POTANTIAL_EMBEDDING_NODES=0;
    ...
    IloInt vn_node=0, cpu_node_requirement=0, link=0, virtual_link_id=0, nb_hops=0, num_candidate_embedding_nodes=0, cost=0;
	...
	cout<<"\t Reading VN requests topology from FILE: "<<f4_vnReqTopo<<endl;
    ...
    //------------------------------------------------------------------------------------------
	cout<<"\t Reading VN requests details from FILE: "<<f7_vnReqLink<<endl;
     ...
     //----------------------------------------------------------------------------------
		cout<<"\t Reading Substrate Topology from FILE: "<<f1_subTopo<<endl;
     ...
     //----------------------------------------------------------------------------------
       ...
       //--------------------------------------------------------------------------------------------------------
        ...
        //------------------------------------------------------------------------------------------------------------
		 cout<<"\t Reading substrate unit costs from FILE: "<<f5_subUnitCost<<endl;
		  ...
		  //------------------------------------------------------------------------------------------
    		...
    		cout<<"\t Reading potantial vNode embedding locations from FILE: "<<f6_vnReqNode<<endl;
		...
		//  substrate_graph_printing(Vect_Substrate_Graph, env, NB_NODE);
		 ...
		 cout<<" Reading embedding paths of reserved requests  from File: "<<prv_f11_ph2EmbeddedVnodes<<endl;
			...
			if (found == 1)
			 {
				 ...
				 Available_cpu_vect[snode-1] = Available_cpu_vect[snode-1] - cpu;
     ...
     //*****************************************************************************************************
             ...
             //------------------------------------------------------------------------
                 cout<<"\t A- Creation of VN embedding variables (test1)"<<endl;
             ...
             //------------------------------------------------------------------------
                 cout<<"\t B- no partially VN Embedding: accept all virtual links or block all (test2)"<<endl;

             no_partially_VN_embedding(Added_Request_Vect, NB_ADD, NB_VNP, x, z, Potantial_Embedding_Nodes_Vect, nb_candidate_embedding_nodes,ILP_model,
				  embedding_trace_x, x_VECT_LENGTH, env);
             ...
             //-----------------------------------------------------------------------
               ...
               VNode_embedding_clash_constraint(VNode_Potantial_Location_Vect, NB_VNP_NODE, embedding_trace_x, x,
				                                x_VECT_LENGTH,z, ILP_model, env);
				cout<<"\t C- A virtual node can be assigned to only one substrate node  (test4)"<<endl;
             ...
             //-----------------------------------------------------------------------------
	            ...
	            substrate_node_cpu_constraint(NB_NODE, embedding_trace_x, x, x_VECT_LENGTH,
					                 ILP_solver, VNode_Potantial_Location_Vect, NB_VNP_NODE, Node_Class_QoS_Vect, Available_cpu_vect, env);

				cout<<"\t D-  Substrate Node CPU Capacity (test7)"<<endl;
             //--------------------------------------------------------------------------------
             ...
             //--------------------------------------------------------------------------------
              ...
              cout<<"\t E- Calculation of the objective function Maximize the PIP Profit (test8)"<<endl;
		  ...
		  if (current_value > EPSELON_1) {
			   ...
			   while (found==0) {
				   ...
				   if (equal_index){
					  ...
					  snode = embedding_trace_x[l].getCandidSnodeId();
					  vnp_id = embedding_trace_x[l].getVnpId();
					  cout<<" virtual node: "<<vnode<<"belongining to VNP: "<< vnp_id<<"was embedded to substrate node: "<<snode<<endl;
	 ...
	 for(i=0;i<nb_accepted_vnode;i++){
	    ...
	    snode  = (IloInt) Vnode_Embedding_Vect[i].getCandidSnodeId();

		cls = (IloInt) search_vnode_class(vnode, vnp_id, VNode_Potantial_Location_Vect, NB_VNP_NODE);

		file8<<vnode<<"    "<<vnp_id <<"    "<<snode<<"    "<<cls<<"    "<<current_period<<endl;

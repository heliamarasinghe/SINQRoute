...
class CLI( Cmd ):
    ...
    def do_hello(self, _line ):
        output("Hello worked from build/linux-x86_64-2.7/mininet/cli.py\n")
        self.mn.hello()

    def do_breaklinks( self, line ):								# ----------------------------------------- breaklinks Function Added by Heli
        output("breaklinks worked from build/linux-x86_64-2.7/mininet/cli.py -- added by Heli\n")
        args = line.split()
        if len(args) == 1:
            #output("breaking link from s3 to s5\n")
            #self.do_link( line="s3 s5 down" )
            #sample(self.mn.links, args[0])
	    linkList = []
	    for link in self.mn.links:
	        node1 = link.intf1.node.name
		node2 = link.intf2.node.name
		#print "node1 = %s\t node2 = %s"%(node1, node2)
		#split("-")[0]
	        #node2 = link.intf2.split("-")[0]
		dev1,num1 = re.match(r"([a-z]+)([0-9]+)", node1, re.I).groups()
		if dev1 != "dc":
		    dev2,num2 = re.match(r"([a-z]+)([0-9]+)", node2, re.I).groups()
		    if dev2 != "dc":
			#print "dev1 = %s \t dev2 = %s"%(dev1, dev2)
	...
	#output("calling hello from  do_breaklinks  \n")
        #self.do_hello(line)

    def do_fixlinks( self, line ):								# ------------------------------------------ fixlinks Function Added by Heli
    ...
    def do_link( self, line ):
        ...
        if len(args) != 3:
            ...
            output("Bringing link between %s and %s %s\n"%(args[0], args[1], args[2]))		# ---------------------------------------------------Line Added by Heli
    ...
    def default( self, line ):
        ...
        if first in self.mn:
            ...
            error( '*** Unknown command -- Heli build/lib.linux: %s\n' % line )
